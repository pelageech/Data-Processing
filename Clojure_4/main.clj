...
(defn notify-msg
  ...
  [state ware storage-atom]                                                      ;   [[ПЕРЕМЕННЫЕ]]
  (let [bill (state :bill)                                                     ; нужное кол-во ware-ресов
        buffer (state :buffer)                                                 ; буфер с ресами
        needed_amount (- (bill ware) (buffer ware))                            ; сколько нам надо ware-ресов для производства
        [a_old a_new] (swap-vals! storage-atom #(- % (min % needed_amount)))   ; забираем со склада ресурсы, но не более, чем достаточно для производства 1 единицы продукта
        new_buffer (assoc buffer ware (+ (buffer ware) (- a_old a_new)))]      ; создаем буфер, меняем количество, которое нужно для производства единицы продукта (>= 0)
    (if (= bill new_buffer)                                                    ; [[УСЛОВИЕ]] если можем произвести
      (do
        (println "ABOBA" ((state :target-storage) :ware) (state :buffer) new_buffer)
        (Thread/sleep (state :duration))                                       ; имитация работы
        (send ((state :target-storage) :worker) supply-msg (state :amount))    ; уведомляем работника на стороне о том, что можно производить
        (assoc state :buffer (reduce-kv (fn [acc k _] (assoc acc k 0))         ; обнуляем буфер
                                        {} bill)))
      (assoc state :buffer new_buffer))))                                      ; [[ИНАЧЕ]] просто обновляем буфер
